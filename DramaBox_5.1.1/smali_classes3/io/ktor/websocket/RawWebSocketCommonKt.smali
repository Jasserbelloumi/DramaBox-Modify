.class public final Lio/ktor/websocket/RawWebSocketCommonKt;
.super Ljava/lang/Object;
.source "SourceFile"


# annotations
.annotation runtime Lkotlin/Metadata;
.end annotation


# direct methods
.method public static final O(Lve/I;Lio/ktor/websocket/O;ZLof/O;)Ljava/lang/Object;
    .locals 10
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lve/I;",
            "Lio/ktor/websocket/O;",
            "Z",
            "Lof/O<",
            "-",
            "Lkotlin/Unit;",
            ">;)",
            "Ljava/lang/Object;"
        }
    .end annotation

    .line 1
    .line 2
    instance-of v0, p3, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;

    .line 3
    .line 4
    if-eqz v0, :cond_0

    .line 5
    move-object v0, p3

    .line 6
    .line 7
    check-cast v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;

    .line 8
    .line 9
    iget v1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 10
    .line 11
    const/high16 v2, -0x80000000

    .line 12
    .line 13
    and-int v3, v1, v2

    .line 14
    .line 15
    if-eqz v3, :cond_0

    .line 16
    sub-int/2addr v1, v2

    .line 17
    .line 18
    iput v1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 19
    goto :goto_0

    .line 20
    .line 21
    :cond_0
    new-instance v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;

    .line 22
    .line 23
    .line 24
    invoke-direct {v0, p3}, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;-><init>(Lof/O;)V

    .line 25
    .line 26
    :goto_0
    iget-object p3, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->result:Ljava/lang/Object;

    .line 27
    .line 28
    .line 29
    invoke-static {}, Lpf/dramabox;->io()Ljava/lang/Object;

    .line 30
    move-result-object v1

    .line 31
    .line 32
    iget v2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 33
    .line 34
    const/16 v3, 0x7f

    .line 35
    const/4 v4, 0x1

    .line 36
    .line 37
    const/16 v5, 0x80

    .line 38
    .line 39
    const/16 v6, 0x7e

    .line 40
    const/4 v7, 0x0

    .line 41
    .line 42
    .line 43
    packed-switch v2, :pswitch_data_0

    .line 44
    .line 45
    new-instance p0, Ljava/lang/IllegalStateException;

    .line 46
    .line 47
    const-string p1, "call to \'resume\' before \'invoke\' with coroutine"

    .line 48
    .line 49
    .line 50
    invoke-direct {p0, p1}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    .line 51
    throw p0

    .line 52
    .line 53
    .line 54
    :pswitch_0
    invoke-static {p3}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 55
    .line 56
    goto/16 :goto_d

    .line 57
    .line 58
    :pswitch_1
    iget p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$0:I

    .line 59
    .line 60
    iget-object p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 61
    .line 62
    check-cast p1, Lye/OT;

    .line 63
    .line 64
    iget-object p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 65
    .line 66
    check-cast p2, Lve/I;

    .line 67
    .line 68
    .line 69
    invoke-static {p3}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 70
    .line 71
    goto/16 :goto_b

    .line 72
    .line 73
    :pswitch_2
    iget-boolean p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 74
    .line 75
    iget-object p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 76
    .line 77
    check-cast p1, Lio/ktor/websocket/O;

    .line 78
    .line 79
    iget-object p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 80
    .line 81
    check-cast p2, Lve/I;

    .line 82
    .line 83
    .line 84
    invoke-static {p3}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 85
    .line 86
    goto/16 :goto_9

    .line 87
    .line 88
    :pswitch_3
    iget p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$1:I

    .line 89
    .line 90
    iget p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$0:I

    .line 91
    .line 92
    iget-boolean p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 93
    .line 94
    iget-object v2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 95
    .line 96
    check-cast v2, Lio/ktor/websocket/O;

    .line 97
    .line 98
    iget-object v5, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 99
    .line 100
    check-cast v5, Lve/I;

    .line 101
    .line 102
    .line 103
    invoke-static {p3}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 104
    .line 105
    goto/16 :goto_8

    .line 106
    .line 107
    :pswitch_4
    iget p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$0:I

    .line 108
    .line 109
    iget-boolean p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 110
    .line 111
    iget-object p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 112
    .line 113
    check-cast p1, Lio/ktor/websocket/O;

    .line 114
    .line 115
    iget-object v2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 116
    .line 117
    check-cast v2, Lve/I;

    .line 118
    .line 119
    .line 120
    invoke-static {p3}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 121
    move p3, p2

    .line 122
    move-object p2, p1

    .line 123
    move p1, p0

    .line 124
    move-object p0, v2

    .line 125
    goto :goto_5

    .line 126
    .line 127
    .line 128
    :pswitch_5
    invoke-static {p3}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 129
    .line 130
    .line 131
    invoke-virtual {p1}, Lio/ktor/websocket/O;->dramaboxapp()[B

    .line 132
    move-result-object p3

    .line 133
    array-length p3, p3

    .line 134
    .line 135
    .line 136
    invoke-virtual {p1}, Lio/ktor/websocket/O;->O()Z

    .line 137
    move-result v2

    .line 138
    .line 139
    if-eqz v2, :cond_1

    .line 140
    move v2, v5

    .line 141
    goto :goto_1

    .line 142
    :cond_1
    move v2, v7

    .line 143
    .line 144
    .line 145
    :goto_1
    invoke-virtual {p1}, Lio/ktor/websocket/O;->I()Z

    .line 146
    move-result v8

    .line 147
    .line 148
    if-eqz v8, :cond_2

    .line 149
    .line 150
    const/16 v8, 0x40

    .line 151
    goto :goto_2

    .line 152
    :cond_2
    move v8, v7

    .line 153
    :goto_2
    or-int/2addr v2, v8

    .line 154
    .line 155
    .line 156
    invoke-virtual {p1}, Lio/ktor/websocket/O;->io()Z

    .line 157
    move-result v8

    .line 158
    .line 159
    if-eqz v8, :cond_3

    .line 160
    .line 161
    const/16 v8, 0x20

    .line 162
    goto :goto_3

    .line 163
    :cond_3
    move v8, v7

    .line 164
    :goto_3
    or-int/2addr v2, v8

    .line 165
    .line 166
    .line 167
    invoke-virtual {p1}, Lio/ktor/websocket/O;->l1()Z

    .line 168
    move-result v8

    .line 169
    .line 170
    if-eqz v8, :cond_4

    .line 171
    .line 172
    const/16 v8, 0x10

    .line 173
    goto :goto_4

    .line 174
    :cond_4
    move v8, v7

    .line 175
    :goto_4
    or-int/2addr v2, v8

    .line 176
    .line 177
    .line 178
    invoke-virtual {p1}, Lio/ktor/websocket/O;->l()Lio/ktor/websocket/FrameType;

    .line 179
    move-result-object v8

    .line 180
    .line 181
    .line 182
    invoke-virtual {v8}, Lio/ktor/websocket/FrameType;->getOpcode()I

    .line 183
    move-result v8

    .line 184
    or-int/2addr v2, v8

    .line 185
    int-to-byte v2, v2

    .line 186
    .line 187
    iput-object p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 188
    .line 189
    iput-object p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 190
    .line 191
    iput-boolean p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 192
    .line 193
    iput p3, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$0:I

    .line 194
    .line 195
    iput v4, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 196
    .line 197
    .line 198
    invoke-interface {p0, v2, v0}, Lve/I;->l(BLof/O;)Ljava/lang/Object;

    .line 199
    move-result-object v2

    .line 200
    .line 201
    if-ne v2, v1, :cond_5

    .line 202
    return-object v1

    .line 203
    :cond_5
    move v9, p2

    .line 204
    move-object p2, p1

    .line 205
    move p1, p3

    .line 206
    move p3, v9

    .line 207
    .line 208
    :goto_5
    if-ge p1, v6, :cond_6

    .line 209
    move v2, p1

    .line 210
    goto :goto_6

    .line 211
    .line 212
    .line 213
    :cond_6
    const v2, 0xffff

    .line 214
    .line 215
    if-gt p1, v2, :cond_7

    .line 216
    move v2, v6

    .line 217
    goto :goto_6

    .line 218
    :cond_7
    move v2, v3

    .line 219
    .line 220
    :goto_6
    if-eqz p3, :cond_8

    .line 221
    goto :goto_7

    .line 222
    :cond_8
    move v5, v7

    .line 223
    :goto_7
    or-int/2addr v5, v2

    .line 224
    int-to-byte v5, v5

    .line 225
    .line 226
    iput-object p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 227
    .line 228
    iput-object p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 229
    .line 230
    iput-boolean p3, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 231
    .line 232
    iput p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$0:I

    .line 233
    .line 234
    iput v2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$1:I

    .line 235
    const/4 v8, 0x2

    .line 236
    .line 237
    iput v8, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 238
    .line 239
    .line 240
    invoke-interface {p0, v5, v0}, Lve/I;->l(BLof/O;)Ljava/lang/Object;

    .line 241
    move-result-object v5

    .line 242
    .line 243
    if-ne v5, v1, :cond_9

    .line 244
    return-object v1

    .line 245
    :cond_9
    move-object v5, p0

    .line 246
    move p0, v2

    .line 247
    move-object v2, p2

    .line 248
    move p2, p3

    .line 249
    .line 250
    :goto_8
    if-eq p0, v6, :cond_c

    .line 251
    .line 252
    if-eq p0, v3, :cond_a

    .line 253
    goto :goto_a

    .line 254
    :cond_a
    int-to-long p0, p1

    .line 255
    .line 256
    iput-object v5, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 257
    .line 258
    iput-object v2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 259
    .line 260
    iput-boolean p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 261
    const/4 p3, 0x4

    .line 262
    .line 263
    iput p3, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 264
    .line 265
    .line 266
    invoke-interface {v5, p0, p1, v0}, Lve/I;->JKi(JLof/O;)Ljava/lang/Object;

    .line 267
    move-result-object p0

    .line 268
    .line 269
    if-ne p0, v1, :cond_b

    .line 270
    return-object v1

    .line 271
    :cond_b
    move p0, p2

    .line 272
    move-object p1, v2

    .line 273
    move-object p2, v5

    .line 274
    :goto_9
    move-object v2, p1

    .line 275
    move-object v5, p2

    .line 276
    move p2, p0

    .line 277
    goto :goto_a

    .line 278
    :cond_c
    int-to-short p0, p1

    .line 279
    .line 280
    iput-object v5, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 281
    .line 282
    iput-object v2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 283
    .line 284
    iput-boolean p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->Z$0:Z

    .line 285
    const/4 p1, 0x3

    .line 286
    .line 287
    iput p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 288
    .line 289
    .line 290
    invoke-interface {v5, p0, v0}, Lve/I;->lks(SLof/O;)Ljava/lang/Object;

    .line 291
    move-result-object p0

    .line 292
    .line 293
    if-ne p0, v1, :cond_b

    .line 294
    return-object v1

    .line 295
    .line 296
    .line 297
    :goto_a
    invoke-virtual {v2}, Lio/ktor/websocket/O;->dramaboxapp()[B

    .line 298
    move-result-object p0

    .line 299
    array-length p1, p0

    .line 300
    .line 301
    .line 302
    invoke-static {p0, v7, p1}, Ljava/nio/ByteBuffer;->wrap([BII)Ljava/nio/ByteBuffer;

    .line 303
    move-result-object p1

    .line 304
    .line 305
    const-string p3, "wrap(array, offset, length)"

    .line 306
    .line 307
    .line 308
    invoke-static {p1, p3}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    .line 309
    .line 310
    new-instance p3, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$$inlined$ByteReadPacket$default$1;

    .line 311
    .line 312
    .line 313
    invoke-direct {p3, p0}, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$$inlined$ByteReadPacket$default$1;-><init>([B)V

    .line 314
    .line 315
    .line 316
    invoke-static {p1, p3}, Lye/RT;->dramabox(Ljava/nio/ByteBuffer;Lkotlin/jvm/functions/Function1;)Lye/OT;

    .line 317
    move-result-object p1

    .line 318
    .line 319
    if-ne p2, v4, :cond_e

    .line 320
    .line 321
    sget-object p0, Lkotlin/random/Random;->Default:Lkotlin/random/Random$Default;

    .line 322
    .line 323
    .line 324
    invoke-virtual {p0}, Lkotlin/random/Random$Default;->nextInt()I

    .line 325
    move-result p0

    .line 326
    .line 327
    iput-object v5, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 328
    .line 329
    iput-object p1, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 330
    .line 331
    iput p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->I$0:I

    .line 332
    const/4 p2, 0x5

    .line 333
    .line 334
    iput p2, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 335
    .line 336
    .line 337
    invoke-interface {v5, p0, v0}, Lve/I;->ygh(ILof/O;)Ljava/lang/Object;

    .line 338
    move-result-object p2

    .line 339
    .line 340
    if-ne p2, v1, :cond_d

    .line 341
    return-object v1

    .line 342
    :cond_d
    move-object p2, v5

    .line 343
    .line 344
    .line 345
    :goto_b
    invoke-static {p1, p0}, Lio/ktor/websocket/RawWebSocketCommonKt;->dramabox(Lye/OT;I)Lye/OT;

    .line 346
    move-result-object p1

    .line 347
    move-object v5, p2

    .line 348
    goto :goto_c

    .line 349
    .line 350
    :cond_e
    if-nez p2, :cond_10

    .line 351
    :goto_c
    const/4 p0, 0x0

    .line 352
    .line 353
    iput-object p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$0:Ljava/lang/Object;

    .line 354
    .line 355
    iput-object p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->L$1:Ljava/lang/Object;

    .line 356
    const/4 p0, 0x6

    .line 357
    .line 358
    iput p0, v0, Lio/ktor/websocket/RawWebSocketCommonKt$writeFrame$1;->label:I

    .line 359
    .line 360
    .line 361
    invoke-interface {v5, p1, v0}, Lve/I;->Jvf(Lye/OT;Lof/O;)Ljava/lang/Object;

    .line 362
    move-result-object p0

    .line 363
    .line 364
    if-ne p0, v1, :cond_f

    .line 365
    return-object v1

    .line 366
    .line 367
    :cond_f
    :goto_d
    sget-object p0, Lkotlin/Unit;->dramabox:Lkotlin/Unit;

    .line 368
    return-object p0

    .line 369
    .line 370
    :cond_10
    new-instance p0, Lkotlin/NoWhenBranchMatchedException;

    .line 371
    .line 372
    .line 373
    invoke-direct {p0}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    .line 374
    throw p0

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method

.method public static final dramabox(Lye/OT;I)Lye/OT;
    .locals 6

    .line 1
    const/4 v0, 0x4

    .line 2
    int-to-long v0, v0

    .line 3
    .line 4
    sget-object v2, Lwe/dramaboxapp;->dramabox:Lwe/dramaboxapp;

    .line 5
    .line 6
    .line 7
    invoke-virtual {v2, v0, v1}, Lwe/dramaboxapp;->O(J)Ljava/nio/ByteBuffer;

    .line 8
    move-result-object v0

    .line 9
    const/4 v1, 0x0

    .line 10
    .line 11
    .line 12
    :try_start_0
    invoke-virtual {v0, v1, p1}, Ljava/nio/ByteBuffer;->putInt(II)Ljava/nio/ByteBuffer;

    .line 13
    .line 14
    new-instance p1, Lye/IO;

    .line 15
    const/4 v3, 0x1

    .line 16
    const/4 v4, 0x0

    .line 17
    .line 18
    .line 19
    invoke-direct {p1, v4, v3, v4}, Lye/IO;-><init>(Lio/ktor/utils/io/pool/dramaboxapp;ILkotlin/jvm/internal/DefaultConstructorMarker;)V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_1

    .line 20
    .line 21
    .line 22
    :try_start_1
    invoke-virtual {p0}, Lye/pos;->syu()J

    .line 23
    move-result-wide v3

    .line 24
    long-to-int v3, v3

    .line 25
    .line 26
    :goto_0
    if-ge v1, v3, :cond_0

    .line 27
    .line 28
    .line 29
    invoke-virtual {p0}, Lye/pos;->readByte()B

    .line 30
    move-result v4

    .line 31
    .line 32
    rem-int/lit8 v5, v1, 0x4

    .line 33
    .line 34
    .line 35
    invoke-virtual {v0, v5}, Ljava/nio/ByteBuffer;->get(I)B

    .line 36
    move-result v5

    .line 37
    xor-int/2addr v4, v5

    .line 38
    int-to-byte v4, v4

    .line 39
    .line 40
    .line 41
    invoke-virtual {p1, v4}, Lye/tyu;->slo(B)V

    .line 42
    .line 43
    add-int/lit8 v1, v1, 0x1

    .line 44
    goto :goto_0

    .line 45
    :catchall_0
    move-exception p0

    .line 46
    goto :goto_1

    .line 47
    .line 48
    .line 49
    :cond_0
    invoke-virtual {p1}, Lye/IO;->LLk()Lye/OT;

    .line 50
    move-result-object p0
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    .line 51
    .line 52
    .line 53
    invoke-virtual {v2, v0}, Lwe/dramaboxapp;->dramabox(Ljava/nio/ByteBuffer;)V

    .line 54
    return-object p0

    .line 55
    .line 56
    .line 57
    :goto_1
    :try_start_2
    invoke-virtual {p1}, Lye/tyu;->release()V

    .line 58
    throw p0
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_1

    .line 59
    :catchall_1
    move-exception p0

    .line 60
    .line 61
    .line 62
    invoke-virtual {v2, v0}, Lwe/dramaboxapp;->dramabox(Ljava/nio/ByteBuffer;)V

    .line 63
    throw p0
.end method

.method public static final dramaboxapp(Lio/ktor/utils/io/ByteReadChannel;JILof/O;)Ljava/lang/Object;
    .locals 23
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lio/ktor/utils/io/ByteReadChannel;",
            "JI",
            "Lof/O<",
            "-",
            "Lio/ktor/websocket/O;",
            ">;)",
            "Ljava/lang/Object;"
        }
    .end annotation

    move-object/from16 v0, p0

    move-object/from16 v1, p4

    instance-of v2, v1, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;

    if-eqz v2, :cond_0

    move-object v2, v1

    check-cast v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;

    iget v3, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    const/high16 v4, -0x80000000

    and-int v5, v3, v4

    if-eqz v5, :cond_0

    sub-int/2addr v3, v4

    iput v3, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    goto :goto_0

    :cond_0
    new-instance v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;

    invoke-direct {v2, v1}, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;-><init>(Lof/O;)V

    :goto_0
    iget-object v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->result:Ljava/lang/Object;

    invoke-static {}, Lpf/dramabox;->io()Ljava/lang/Object;

    move-result-object v3

    .line 1
    iget v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    const/4 v5, 0x0

    const/4 v8, 0x1

    packed-switch v4, :pswitch_data_0

    new-instance v0, Ljava/lang/IllegalStateException;

    const-string v1, "call to \'resume\' before \'invoke\' with coroutine"

    invoke-direct {v0, v1}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    throw v0

    :pswitch_0
    iget v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$1:I

    iget v3, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iget-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iget-object v2, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    check-cast v2, Lio/ktor/websocket/FrameType;

    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    move-object/from16 v17, v2

    goto/16 :goto_10

    :pswitch_1
    iget-wide v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$1:J

    iget v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iget-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iget-wide v11, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iget-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    check-cast v13, Lio/ktor/websocket/FrameType;

    iget-object v14, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    check-cast v14, Lio/ktor/utils/io/ByteReadChannel;

    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    goto/16 :goto_e

    :pswitch_2
    iget v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iget-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$1:B

    iget-byte v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iget-wide v10, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iget-object v12, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    check-cast v12, Lio/ktor/websocket/FrameType;

    iget-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    check-cast v13, Lio/ktor/utils/io/ByteReadChannel;

    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    goto/16 :goto_8

    :pswitch_3
    iget v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iget-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$1:B

    iget-byte v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iget-wide v10, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iget-object v12, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    check-cast v12, Lio/ktor/websocket/FrameType;

    iget-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    check-cast v13, Lio/ktor/utils/io/ByteReadChannel;

    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    goto/16 :goto_a

    :pswitch_4
    iget-byte v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iget v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iget-wide v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iget-object v11, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    check-cast v11, Lio/ktor/utils/io/ByteReadChannel;

    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    move-object v13, v11

    move-wide v10, v9

    move v9, v0

    goto :goto_2

    :pswitch_5
    iget v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iget-wide v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iget-object v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    check-cast v4, Lio/ktor/utils/io/ByteReadChannel;

    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    move-object/from16 v22, v1

    move v1, v0

    move-object v0, v4

    move-object/from16 v4, v22

    goto :goto_1

    :pswitch_6
    invoke-static {v1}, Lkotlin/O;->dramaboxapp(Ljava/lang/Object;)V

    .line 2
    iput-object v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    move-wide/from16 v9, p1

    iput-wide v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    move/from16 v1, p3

    iput v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iput v8, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    invoke-interface {v0, v2}, Lio/ktor/utils/io/ByteReadChannel;->IO(Lof/O;)Ljava/lang/Object;

    move-result-object v4

    if-ne v4, v3, :cond_1

    return-object v3

    :cond_1
    :goto_1
    check-cast v4, Ljava/lang/Number;

    invoke-virtual {v4}, Ljava/lang/Number;->byteValue()B

    move-result v4

    .line 3
    iput-object v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    iput-wide v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iput v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iput-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    const/4 v11, 0x2

    iput v11, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    invoke-interface {v0, v2}, Lio/ktor/utils/io/ByteReadChannel;->IO(Lof/O;)Ljava/lang/Object;

    move-result-object v11

    if-ne v11, v3, :cond_2

    return-object v3

    :cond_2
    move-object v13, v0

    move/from16 v22, v4

    move v4, v1

    move-object v1, v11

    move-wide v10, v9

    move/from16 v9, v22

    :goto_2
    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->byteValue()B

    move-result v0

    and-int/lit8 v1, v9, 0xf

    if-nez v1, :cond_4

    if-eqz v4, :cond_3

    goto :goto_3

    .line 4
    :cond_3
    new-instance v0, Lio/ktor/websocket/ProtocolViolationException;

    const-string v1, "Can\'t continue finished frames"

    invoke-direct {v0, v1}, Lio/ktor/websocket/ProtocolViolationException;-><init>(Ljava/lang/String;)V

    throw v0

    :cond_4
    :goto_3
    if-nez v1, :cond_5

    move v12, v4

    goto :goto_4

    :cond_5
    move v12, v1

    .line 5
    :goto_4
    sget-object v14, Lio/ktor/websocket/FrameType;->Companion:Lio/ktor/websocket/FrameType$dramabox;

    invoke-virtual {v14, v12}, Lio/ktor/websocket/FrameType$dramabox;->dramabox(I)Lio/ktor/websocket/FrameType;

    move-result-object v14

    if-eqz v14, :cond_1c

    if-eqz v1, :cond_7

    if-eqz v4, :cond_7

    .line 6
    invoke-virtual {v14}, Lio/ktor/websocket/FrameType;->getControlFrame()Z

    move-result v1

    if-eqz v1, :cond_6

    goto :goto_5

    .line 7
    :cond_6
    new-instance v0, Lio/ktor/websocket/ProtocolViolationException;

    const-string v1, "Can\'t start new data frame before finishing previous one"

    invoke-direct {v0, v1}, Lio/ktor/websocket/ProtocolViolationException;-><init>(Ljava/lang/String;)V

    throw v0

    :cond_7
    :goto_5
    and-int/lit16 v1, v9, 0x80

    if-eqz v1, :cond_8

    move v1, v8

    goto :goto_6

    :cond_8
    const/4 v1, 0x0

    .line 8
    :goto_6
    invoke-virtual {v14}, Lio/ktor/websocket/FrameType;->getControlFrame()Z

    move-result v4

    if-eqz v4, :cond_a

    if-eqz v1, :cond_9

    goto :goto_7

    .line 9
    :cond_9
    new-instance v0, Lio/ktor/websocket/ProtocolViolationException;

    const-string v1, "control frames can\'t be fragmented"

    invoke-direct {v0, v1}, Lio/ktor/websocket/ProtocolViolationException;-><init>(Ljava/lang/String;)V

    throw v0

    :cond_a
    :goto_7
    and-int/lit8 v4, v0, 0x7f

    const/16 v12, 0x7e

    if-eq v4, v12, :cond_d

    const/16 v12, 0x7f

    if-eq v4, v12, :cond_b

    int-to-long v6, v4

    move v4, v9

    move-wide v11, v10

    move-wide v9, v6

    move-object/from16 v22, v14

    move-object v14, v13

    move-object/from16 v13, v22

    goto :goto_b

    .line 10
    :cond_b
    iput-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    iput-object v14, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    iput-wide v10, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iput-byte v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iput-byte v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$1:B

    iput v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    const/4 v4, 0x4

    iput v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    invoke-interface {v13, v2}, Lio/ktor/utils/io/ByteReadChannel;->ppo(Lof/O;)Ljava/lang/Object;

    move-result-object v4

    if-ne v4, v3, :cond_c

    return-object v3

    :cond_c
    move-object v12, v14

    move-object/from16 v22, v4

    move v4, v0

    move v0, v1

    move-object/from16 v1, v22

    :goto_8
    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->longValue()J

    move-result-wide v6

    :goto_9
    move v1, v0

    move v0, v4

    move v4, v9

    move-object v14, v13

    move-object v13, v12

    move-wide v11, v10

    move-wide v9, v6

    goto :goto_b

    .line 11
    :cond_d
    iput-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    iput-object v14, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    iput-wide v10, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iput-byte v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iput-byte v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$1:B

    iput v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    const/4 v4, 0x3

    iput v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    invoke-interface {v13, v2}, Lio/ktor/utils/io/ByteReadChannel;->Jkl(Lof/O;)Ljava/lang/Object;

    move-result-object v4

    if-ne v4, v3, :cond_e

    return-object v3

    :cond_e
    move-object v12, v14

    move-object/from16 v22, v4

    move v4, v0

    move v0, v1

    move-object/from16 v1, v22

    :goto_a
    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->shortValue()S

    move-result v1

    int-to-long v6, v1

    const-wide/32 v15, 0xffff

    and-long/2addr v6, v15

    goto :goto_9

    .line 12
    :goto_b
    invoke-virtual {v13}, Lio/ktor/websocket/FrameType;->getControlFrame()Z

    move-result v6

    if-eqz v6, :cond_10

    const-wide/16 v6, 0x7d

    cmp-long v6, v9, v6

    if-gtz v6, :cond_f

    goto :goto_c

    .line 13
    :cond_f
    new-instance v0, Lio/ktor/websocket/ProtocolViolationException;

    const-string v1, "control frames can\'t be larger than 125 bytes"

    invoke-direct {v0, v1}, Lio/ktor/websocket/ProtocolViolationException;-><init>(Ljava/lang/String;)V

    throw v0

    :cond_10
    :goto_c
    and-int/lit16 v0, v0, 0x80

    if-eqz v0, :cond_11

    move v0, v8

    goto :goto_d

    :cond_11
    const/4 v0, 0x0

    :goto_d
    if-ne v0, v8, :cond_13

    .line 14
    iput-object v14, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    iput-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    iput-wide v11, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$0:J

    iput-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iput v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iput-wide v9, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->J$1:J

    const/4 v0, 0x5

    iput v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    invoke-interface {v14, v2}, Lio/ktor/utils/io/ByteReadChannel;->yiu(Lof/O;)Ljava/lang/Object;

    move-result-object v0

    if-ne v0, v3, :cond_12

    return-object v3

    :cond_12
    move/from16 v22, v1

    move-object v1, v0

    move/from16 v0, v22

    :goto_e
    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->intValue()I

    move-result v1

    move/from16 v22, v1

    move v1, v0

    move/from16 v0, v22

    goto :goto_f

    :cond_13
    if-nez v0, :cond_1b

    const/4 v0, -0x1

    :goto_f
    const-wide/32 v6, 0x7fffffff

    cmp-long v6, v9, v6

    if-gtz v6, :cond_1a

    cmp-long v6, v9, v11

    if-gtz v6, :cond_1a

    long-to-int v6, v9

    .line 15
    iput-object v13, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$0:Ljava/lang/Object;

    iput-object v5, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->L$1:Ljava/lang/Object;

    iput-byte v4, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->B$0:B

    iput v1, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$0:I

    iput v0, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->I$1:I

    const/4 v7, 0x6

    iput v7, v2, Lio/ktor/websocket/RawWebSocketCommonKt$readFrame$1;->label:I

    invoke-interface {v14, v6, v2}, Lio/ktor/utils/io/ByteReadChannel;->yyy(ILof/O;)Ljava/lang/Object;

    move-result-object v2

    if-ne v2, v3, :cond_14

    return-object v3

    :cond_14
    move v3, v1

    move-object v1, v2

    move-object/from16 v17, v13

    .line 16
    :goto_10
    check-cast v1, Lye/OT;

    const/4 v2, -0x1

    if-ne v0, v2, :cond_15

    goto :goto_11

    .line 17
    :cond_15
    invoke-static {v1, v0}, Lio/ktor/websocket/RawWebSocketCommonKt;->dramabox(Lye/OT;I)Lye/OT;

    move-result-object v1

    .line 18
    :goto_11
    sget-object v15, Lio/ktor/websocket/O;->ll:Lio/ktor/websocket/O$O;

    if-eqz v3, :cond_16

    move/from16 v16, v8

    const/4 v0, 0x0

    goto :goto_12

    :cond_16
    const/4 v0, 0x0

    const/16 v16, 0x0

    .line 19
    :goto_12
    invoke-static {v1, v0, v8, v5}, Lye/djd;->l(Lye/OT;IILjava/lang/Object;)[B

    move-result-object v18

    and-int/lit8 v1, v4, 0x40

    if-eqz v1, :cond_17

    move/from16 v19, v8

    goto :goto_13

    :cond_17
    move/from16 v19, v0

    :goto_13
    and-int/lit8 v1, v4, 0x20

    if-eqz v1, :cond_18

    move/from16 v20, v8

    goto :goto_14

    :cond_18
    move/from16 v20, v0

    :goto_14
    and-int/lit8 v1, v4, 0x10

    if-eqz v1, :cond_19

    move/from16 v21, v8

    goto :goto_15

    :cond_19
    move/from16 v21, v0

    .line 20
    :goto_15
    invoke-virtual/range {v15 .. v21}, Lio/ktor/websocket/O$O;->dramabox(ZLio/ktor/websocket/FrameType;[BZZZ)Lio/ktor/websocket/O;

    move-result-object v0

    return-object v0

    .line 21
    :cond_1a
    new-instance v0, Lio/ktor/websocket/FrameTooBigException;

    invoke-direct {v0, v9, v10}, Lio/ktor/websocket/FrameTooBigException;-><init>(J)V

    throw v0

    .line 22
    :cond_1b
    new-instance v0, Lkotlin/NoWhenBranchMatchedException;

    invoke-direct {v0}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    throw v0

    .line 23
    :cond_1c
    new-instance v0, Ljava/lang/IllegalStateException;

    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1}, Ljava/lang/StringBuilder;-><init>()V

    const-string v2, "Unsupported opcode: "

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v1, v12}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-direct {v0, v1}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    throw v0

    nop

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method
